    ...
    <div class="content">
        <div class="limit-block">
            <div>
                ...
                <?php
                if (conf["payment-for-results"] and $_SESSION['usertype'] != 1) {
                    ...
                    if (!$link) {
                        ...
                        $sql = mysqli_fetch_array(mysqli_query($link,
                            "SELECT COUNT(*) FROM `results` WHERE `forId`=(SELECT `id` FROM `users` WHERE `userkey`='$userkey')"));
                        if ($sql['COUNT(*)'] == 0) {
